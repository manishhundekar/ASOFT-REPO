...
class AddPatient extends Component {
    ...
    submitPatForm = (e) => {
        e.preventDefault()
        axios.post("http://localhost:9050/patient" , this.state.patient)
        .then((res)=>
        {
        console.log(res)
        if(res.status === 200){
    ...
    render()
    {
        return (
            ...
            <hr />
            <h3 align="center" className="my-1">Add Patient</h3>
            <form>
            ...
            <div>
                <button type="submit" className="btn btn-primary" onClick={this.submitPatForm} id="add" style={{width:100 + '%'}}>Add Patient</button>
            </div>
            </form>
        </div>
        </div>
